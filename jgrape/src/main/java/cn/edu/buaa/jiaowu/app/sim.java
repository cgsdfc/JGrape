...
public class sim extends worker <Integer, graph> {
      ...
      ArrayList<frag> fragments, ArrayList<graph> queries)
  {
    ...
    for (int i=0;i<nfrag;++i) {
      this.simset.add(new HashMap<Integer, HashSet<Integer>> ());
      this.removed.add(new HashMap<Integer, HashSet<Integer>> ());
      this.tset.add(new HashMap<Integer, HashSet<Integer>> ());
      this.presim.add(new HashMap<Integer, HashSet<Integer>> ());
      this.tag2Id.add(new HashMap<Integer,Integer> ());
      this.count.add(new HashMap<Integer,ArrayList<Integer>> ());
      this.active.add(new HashMap<Integer,Boolean> ());
      this.prev.add(new HashSet<Integer> ());
    }
  ...
  protected void peval(frag fragment, 
    ...
    for (node N:query.getnodes()) {
      ...
      simset.put(uid, new HashSet<Integer> ());
      removed.put(uid, new HashSet<Integer> ());
    }
  ...
  protected void inceval (frag fragment,
    ...
    for (node N:query.getnodes()){
      int uid=N.getvid();
      delta.put(uid, new HashSet<Integer> ());
    }
